// Modals: Upload, Diff, TestGen, PR
const { useState: useStateM, useEffect: useEffectM, useRef: useRefM } = React;

function UploadModal({ mode, onClose, onOpenProject }) {
  const [tab, setTab] = useStateM(mode || 'upload');
  useEffectM(() => setTab(mode || 'upload'), [mode]);
  return (
      ...
      <div className="modal-body">
        ...
        {tab === 'paste' && (
          <div>
            ...
            <textarea className="pr-textarea mono" style={{ minHeight: 180, fontFamily: 'JetBrains Mono, monospace', fontSize: 12.5 }} placeholder="Paste your code here…" defaultValue={`@Service\npublic class OrderService {\n    // …\n}`}/>
          </div>
        )}
        ...
        {tab === 'git' && (
          <div>
            ...
            <div className="git-connect-list">
              {[{r:'acme/orders-api', b:'main • 2h ago'},{r:'acme/billing-service', b:'feat/refunds • 1d ago'},{r:'acme/notify-lambda', b:'main • 3d ago'}].map((x, i) => (
...
function DiffModal({ suggestion, onClose, onAccept, onGenerateTests }) {
  const [viewFull, setViewFull] = useStateM(true);
  const [comment, setComment] = useStateM('');
    ...
    <ModalShell onClose={onClose} sizeClass="diff">
      ...
      <div style={{ padding: '12px 20px', borderTop: '1px solid var(--line)', background: 'var(--panel-2)' }}>
        <div className="composer-box" style={{ background: 'var(--panel)' }}>
          <textarea rows={1} placeholder="Refine: e.g. use Collections.emptyList() instead of Optional, or add a log statement…"
...
function TypingTests({ onDone }) {
  ...
  const [curLine, setCurLine] = useStateM(0);
  const [curCol, setCurCol] = useStateM(0);
  ...
  const colorize = (line) => {
    ...
    const patterns = [
      { re: /^(\s*)(@\w+)/, cls: 'anno' },
      { re: /\b(class|void|public|private|return|new|void|static|import)\b/, cls: 'kw' },
    ];
    // simple highlight
    return line
      .replace(/("[^"]*")/g, '§str§$1§/str§')
      .replace(/\b(class|void|public|private|return|new|static|import|final)\b/g, '§kw§$1§/kw§')
      .replace(/(@\w+)/g, '§anno§$1§/anno§')
      .split(/§(\/?\w+)§/).reduce((acc, p, i) => {
        if (i % 2 === 0) acc.push(p);
        else if (!p.startsWith('/')) acc._open = p;
        return acc;
      }, []);
  };

  // Simpler: render with classes via dangerously
  const html = (line) =>
    line
      .replace(/("[^"]*")/g, '<span class="str">$1</span>')
      .replace(/(@\w+)/g, '<span class="anno">$1</span>')
      .replace(/\b(class|void|public|private|return|new|static|import|final|when|any|void|thenAnswer|assertThat)\b/g, '<span class="kw">$1</span>')
...
function TestsModal({ onClose, onSave }) {
  const [step, setStep] = useStateM(0); // 0 gen, 1 run, 2 coverage
  const [testStates, setTestStates] = useStateM(TEST_CASES.map(() => 'pending'));
  const [pct, setPct] = useStateM(0);

  const goToRun = () => {
    setStep(1);
    // simulate running: one at a time
      ...
      <div className="modal-foot">
        {step < 2 ? (
          ...
          <>
            ...
            <span className="spacer"></span>
            <button className="btn" onClick={() => onSave('download')}><I.Upload size={12} style={{ transform: 'rotate(180deg)' }}/> Download project</button>
...
function PrModal({ onClose, onSubmitted }) {
    ...
    <ModalShell onClose={onClose} sizeClass="pr">
      ...
      <div className="modal-foot">
        ...
        <button className="btn" onClick={() => { onClose(); onSubmitted && onSubmitted('download'); }}>
          <I.Upload size={12} style={{ transform: 'rotate(180deg)' }}/> Download project
        </button>
...
Object.assign(window, { UploadModal, DiffModal, TestsModal, PrModal });
