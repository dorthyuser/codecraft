// Chat panel + workspace (code viewer)
...
function ChatPanel({ activity, language, onPreviewSuggestion, selectedSugg, setSelectedSugg }) {
  ...
  const [thinking, setThinking] = useStateW(true);
  const [revealed, setRevealed] = useStateW(0);
  const [draft, setDraft] = useStateW('');
  const bodyRef = useRefW(null);

  useEffectW(() => {
    setThinking(true); setRevealed(0);
        ...
        {thinking ? (
          <div className="msg ai">
            ...
            <div className="content">
              ...
              <div className="ai-thinking">Reading OrderService.java, OrderRequest.java<span className="dots"><span></span><span></span><span></span></span></div>
            </div>
          </div>
        ) : (
          <div className="msg ai">
...
function ChatComposer({ draft, setDraft }) {
  ...
  const run = () => {
    const chosen = INSTRUCTION_BUTTONS.filter(b => active.includes(b.id));
    const hasAnyInput = draft.trim().length > 0 || chosen.length > 0;
      ...
      <div className="instr-row">
        {INSTRUCTION_BUTTONS.map(b => (
          <button key={b.id}
                  ...
                  onClick={() => toggle(b.id)}
                  title={b.prompt}>
            {React.createElement(I[b.icon], { size: 11 })}
            {b.label}
...
function Workspace({ project, onBack, onOpenDiff, onOpenTests, onOpenPr, language, onChangeLang, activity, setActivity, selectedSugg, setSelectedSugg, onToast }) {
      ...
      <div className="editor-wrap">
        <div className="editor-tabs">
          <button className="ed-tab active">
            <span className={`lang-chip ${language}`} style={{ width: 16, height: 16, fontSize: 9, borderRadius: 3 }}>{language === 'java' ? 'J' : language === 'cs' ? 'C#' : language === 'node' ? 'N' : 'A'}</span>
            {sample.file}
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <select value={language} onChange={e => onChangeLang(e.target.value)}
                    style={{
                      ...
                      background: 'var(--panel)', color: 'var(--ink-2)', fontSize: 12,
                      fontFamily: 'inherit'
                    }}>
            ...
            {ACTIVITIES.map(a => (
              <button key={a.id}
                ...
                {React.createElement(I[a.icon], { size: 12 })}
                {a.label}
              </button>
            ))}
          </div>
