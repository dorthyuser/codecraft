// Dashboard screen: entry cards + recent projects grid
const { useState: useStateD } = React;

function Dashboard({ onOpenProject, onUpload }) {
        ...
        {filtered.map(p => (
          <div key={p.id} className="proj-card" onClick={() => onOpenProject(p)}>
            ...
            <div className="proj-stats">
              <span className="stat"><span className={`dot ${p.status}`}></span> {p.status === 'g' ? 'Healthy' : p.status === 'y' ? 'Needs attention' : 'Issues'}</span>
