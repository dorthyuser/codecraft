...
export const SUGGESTIONS = {
  fix: [
    { id: 's1', badge: 'fix', title: 'Guard against null items', desc: 'OrderRequest.getItems() may return null when deserialized from a request with no items field. Use Optional.ofNullable and an empty stream fallback.', impact: 'Fixes NPE on line 13', confidence: 98, files: 1 },
    { id: 's2', badge: 'fix', title: 'Validate request at controller', desc: 'Add @Valid on the controller parameter and @NotNull on OrderRequest.items so malformed requests are rejected earlier with a 400.', impact: '1 file, 6 lines', confidence: 84, files: 2 },
    { id: 's3', badge: 'fix', title: 'Return 422 on empty cart', desc: 'Throw EmptyCartException → maps to 422 via GlobalExceptionHandler. Keeps domain rules out of the stream pipeline.', impact: 'Behavioral', confidence: 72, files: 2 },
  ],
  improve: [
